...
#checks if a bomb has been clicked and shows end game screen                
def game_over(win, boxes):
    for box in boxes:
        if box.type == 'mine':
            box.image = mine
    draw_end(win, 'Game Over', 30, BLACK)
    button(win, 'Retry', 30, BLACK, 600, 450, 100, 50, main(win))
...
#draws a button that performs given action when pressed
def button(surface, text, size, color, x, y, w, h, action = None):
    mouse = pygame.mouse.get_pos()
    click = pygame.mouse.get_pressed()
    font = pygame.font.SysFont('symbola', size, bold=True) 
    label = font.render(text, 1, color)
    pygame.draw.rect(surface, BLACK, (x,y, w, h))
    pygame.draw.rect(surface, WHITE, (x+5 , y+5, w-10, h-10))
...
def main(win):
    ...
    while running:
        ...
        #constantly checks if player won and updates game
        if win_con(boxes):
            you_win(win)
        update_sq(win, boxes)
